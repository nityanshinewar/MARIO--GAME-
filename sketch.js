...
WinnerImg=loadImage("pictures/win.png");
marioWinImg=loadAnimation("pictures/mariowin.png");
}
...
function setup(){
  ...
  Restart=createSprite(750,180,20,20);
  ...
  mario.addAnimation("marioWinImg",marioWinImg);
  invisibleground=createSprite(500,742,1850,138);
  invisibleground.visible=false;
  //mario.debug=true;
  mario.setCollider("rectangle",0,0,25,35)

  Win=createSprite(750,400,40,40)
  Win.addImage("WinnerImg",WinnerImg);
  Win.visible=false;
...
function draw(){
  ...
  if(GameState===Play){
   ...
   if(GameState===Winner){
    ...
    Win.visible=true;
    //mario.velocityY=mario.velocityY+0.8;
    mario.changeAnimation("marioWinImg",marioWinImg);
    mario.scale=0.2;
    mario.velocityY=0;
    mario.y=595;
